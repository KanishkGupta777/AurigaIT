...
## Cleaning a messy seat-class price list

A separate, self-contained problem from the pricing engine: given a raw
list of `{name, price}` entries scraped from a spreadsheet, produce a
correct, de-duplicated price list plus a report of what happened to every
row. Kept as its own module (`pricing/price_import.py`) rather than bolted
onto `Catalog`, since importing/cleaning a price list and *using* a price
list are different concerns with different failure modes.

Design choices:

- **Price parsing** strips known currency markers (`Rs.`, `Rs`, `INR`,
  `₹`), a trailing `/-`, and thousands commas, then parses what's left as
  a `Decimal`. Anything that still doesn't parse (blank, or genuine
  garbage like `"abc"`) is rejected with the specific reason, not silently
  coerced to zero.
- **Case-insensitive de-duplication**: `"Silver"`, `"silver"`, `"SILVER"`
  all key on `.lower()`. The *first* occurrence sets the canonical
  spelling (`.title()`-cased) and its price; later occurrences that agree
  on price are merged silently (reported under `deduplicated`, not
  treated as an error). Later occurrences that **disagree** on price are
  a genuinely ambiguous situation — I can't know which price is "right" —
  so rather than silently overwrite or average them, the first-seen price
  is kept and the conflicting later row is rejected with an explicit
  reason naming both prices, so a human can go verify the real number.
- **Negative prices are always rejected**, regardless of dedup status.
- The importer is a pure function returning a structured report
  (`imported` / `deduplicated` / `rejected`, each with a reason where
  relevant) — it never mutates global state, so it's trivially unit
  tested with a single messy list covering every case at once
  (`tests/test_price_import.py::test_messy_list_end_to_end`).
- Exposed at `POST /catalog/import-price-list`, deliberately taking a loose
  `list` rather than a strict pydantic schema for the entries — a strict
  schema would reject the messy input before the cleaning logic ever saw
  it, which defeats the point of the exercise. It does not write into the
  live `Catalog`; wiring the cleaned output into an actual `Show`'s tiers
  is a follow-up step a human approves, not something this endpoint does
  automatically.

## Poster art on the booking page

The brief asked for artwork next to each show. I didn't hotlink or embed
actual studio/streaming promotional images (movie posters and official
stills are copyrighted media) — instead each show gets a small,
originally-styled placeholder panel (`.poster-card` in `index.html`): a
CSS gradient themed per show plus a plain Unicode icon, non-clickable
(`pointer-events: none`) as required, swapped by `show_id` in
`POSTER_THEMES`. It's intentionally simple and clearly labelled as
placeholder art in its own caption, with a comment showing exactly where
to drop in real, properly-licensed artwork if you have the rights to it.
...
- Configurable discount stacking order via `PricingConfig`, rather than a
  fixed order in code.
- Wiring the price-list importer's output directly into `Catalog`/`Show`
  construction (currently it only produces a report; turning that into
  live `SeatTier`s is a natural next step).
