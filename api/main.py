...
from pricing.price_import import import_price_list
...
# ---------------------------------------------------------------------------
# Messy price-list import ("the twist")
# ---------------------------------------------------------------------------

class PriceListImportRequest(BaseModel):
    # Deliberately loose typing (List[Dict[str, Any]], not a strict model)
    # -- the entire point of this endpoint is that the input is messy and
    # untyped (blank strings, numbers-as-strings, stray whitespace, etc).
    # A strict pydantic schema would reject the mess before we even get a
    # chance to clean it, which defeats the purpose.
    entries: list


@app.post("/catalog/import-price-list")
def import_price_list_endpoint(req: PriceListImportRequest):
    """
    Clean a messy seat-class price list (duplicate names in different
    cases, inconsistent price formats, blanks, negatives) and report
    exactly what was imported, de-duplicated, and rejected -- and why.

    This does NOT touch the live catalog; it's a standalone cleaning/
    reporting utility. A real system would show this report to a human
    (e.g. the box-office manager who uploaded the spreadsheet) before
    anyone commits the cleaned list as the new price list.
    """
    report = import_price_list(req.entries)
